 ...
 */
package quests.Q19904_LevelUpTo76;
...
import quests.Q10341_MemoryOfTheGloriousPast4.Q10341_MemoryOfTheGloriousPast4;

/**
 * @author Magik
 */
public class Q19904_LevelUpTo76 extends Quest
{
	private static final int QUEST_ID = 19904;
	
	public Q19904_LevelUpTo76()
	{
		super(QUEST_ID);
	}
	...
	public String onEvent(String event, Npc npc, Player player)
	{
		switch (event)
		{
			...
			case "COMPLETE":
			{
				...
				final QuestState nextQuestState = player.getQuestState(Q10341_MemoryOfTheGloriousPast4.class.getSimpleName());
				if (nextQuestState == null)
				{
					player.sendPacket(new ExQuestDialog(10341, QuestDialogType.ACCEPT));
				}
	...
	public void onPlayerLevelChange(OnPlayerLevelChanged event)
	{
		...
		else if ((questState != null) && questState.isStarted() && !questState.isCompleted() && (player.getLevel() == 76))
		{
			questState.setCount(getQuestData().getGoal().getCount());
			questState.setCond(QuestCondType.DONE);
			player.sendPacket(new ExQuestNotification(questState));
