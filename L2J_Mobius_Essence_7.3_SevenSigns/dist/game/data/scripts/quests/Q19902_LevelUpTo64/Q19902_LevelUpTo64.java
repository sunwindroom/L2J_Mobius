 ...
 */
package quests.Q19902_LevelUpTo64;
...
import quests.Q10342_TracesOfBattle1.Q10342_TracesOfBattle1;

/**
 * @author Magik
 */
public class Q19902_LevelUpTo64 extends Quest
{
	private static final int QUEST_ID = 19902;
	
	public Q19902_LevelUpTo64()
	{
		super(QUEST_ID);
	}
	...
	public String onEvent(String event, Npc npc, Player player)
	{
		switch (event)
		{
			...
			case "COMPLETE":
			{
				...
				final QuestState nextQuestState = player.getQuestState(Q10342_TracesOfBattle1.class.getSimpleName());
				if (nextQuestState == null)
				{
					player.sendPacket(new ExQuestDialog(10342, QuestDialogType.ACCEPT));
				}
	...
	public void onPlayerLevelChange(OnPlayerLevelChanged event)
	{
		...
		else if ((questState != null) && questState.isStarted() && !questState.isCompleted() && (player.getLevel() == 64))
		{
			questState.setCount(getQuestData().getGoal().getCount());
			questState.setCond(QuestCondType.DONE);
			player.sendPacket(new ExQuestNotification(questState));
